...
public class Node extends UnicastRemoteObject implements ChordNodeInterface {
	...
	public boolean requestWriteOperation(Message message) throws RemoteException {
		...
		boolean electionresult = multicastMessage(message); // request for write permission from N/2 + 1 fingerTable
															// (majority)
	...
	public boolean requestReadOperation(Message message) throws RemoteException {
		...
		// wants to access resource - multicast clock + message to other processes
		WANTS_TO_ENTER_CS = true;
		boolean electionresult = multicastMessage(message); // request for read permission from N/2 + 1 fingerTable
															// (majority)
	...
	private boolean multicastMessage(Message message) throws AccessException, RemoteException {
		...
		this.queueACK.clear();
		for (Message activenodes : this.activenodesforfile) {
			...
			try {
				...
				this.queueACK.add(node.onMessageReceived(message));
			} catch (NotBoundException e) {

				// e.printStackTrace();
			}
		}
		return this.majorityAcknowledged(); // change to the election result
	}
	...
	public Message onMessageReceived(Message message) throws RemoteException {
		 ...
		 */
		else if (this.WANTS_TO_ENTER_CS && message.getClock() < this.counter) {
			message.setAcknowledged(true);
			this.acquireLock();
		}
	...
	public boolean majorityAcknowledged() throws RemoteException {
		...
		return queueACK.stream().filter(x -> x.isAcknowledged() == true).count() >= quorum; // change this to the result
																							// of the vote
	}
	...
	public void onReceivedUpdateOperation(Message message) throws RemoteException {
		Operations op = new Operations(this, message, this.activenodesforfile);
	...
	public void multicastUpdateOrReadReleaseLockOperation(Message message) throws RemoteException {
		this.activenodesforfile.remove(message);
		...
		// fingerTable (voters)
		// otherwise if this is a READ operation multicast releaselocks to the
		// fingerTable (voters)
	}
	...
	public void multicastVotersDecision(Message message) throws RemoteException {
		...
		// multicast voters decision to the rest of the fingerTable (i.e
		// activenodesforfile)
